/**
 * Weather API - Cloudflare Pages Function
 * ශ්‍රී ලංකාවේ සියලු දිස්ත්‍රික්ක සඳහා කාලගුණ දත්ත ලබා ගැනීම
 */
...
function mockData(d) {
  const baseTemps = { Colombo:29,Gampaha:29,Kalutara:29,Kandy:24,Matale:26,'Nuwara Eliya':16,Galle:28,Matara:29,Hambantota:32,Jaffna:33,Kilinochchi:31,Mannar:33,Vavuniya:30,Mullaitivu:29,Batticaloa:29,Ampara:30,Trincomalee:31,Kurunegala:28,Puttalam:32,Anuradhapura:31,Polonnaruwa:30,Badulla:22,Monaragala:28,Ratnapura:27,Kegalle:26 };
...
async function realData(d, apiKey) {
  if (!apiKey || apiKey === 'YOUR_API_KEY_HERE') return mockData(d);
  try {
...
export async function onRequest({ request, env }) {
  ...
  const getData = d => realData(d, apiKey);

  if (path === '/api/districts') return json({ success: true, districts: DISTRICTS });
  ...
  if (singleMatch) {
    const d = DISTRICTS.find(x => x.id === singleMatch[1]);
    if (!d) return json({ success: false, error: 'District not found' }, 404);
    return json({ success: true, data: await getData(d) });
  }
